...
Mesh::Geometry GeometryIcosahedron::create(float radius, unsigned int detail)
{
	...
	for (int i = 0; i < detail; ++i)
	{
		...
		for (size_t j = 0; j < indices.size(); j += 3)
		{
			...
			glm::vec3 v12 = glm::normalize(vertices[v1] + vertices[v2]) * radius;
			glm::vec3 v23 = glm::normalize(vertices[v2] + vertices[v3]) * radius;
			glm::vec3 v31 = glm::normalize(vertices[v3] + vertices[v1]) * radius;
	...
	for (size_t i = 0; i < vertices.size(); ++i)
	{
		...
		float phi = std::acos(vertices[i].y / radius);
